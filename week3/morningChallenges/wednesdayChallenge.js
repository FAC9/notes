...
var getOrgPremisesId = function(orgID, cb) {
    mockAPI.getOrganisation(orgID, function(error, result) {
        if (error) {
            return cb(error);
        }
        var premId = result.premises.id;
        cb(null, premId)
    })
}
var getPremisesName = function(prem, cb) {
    mockAPI.getPremises(prem, function(error, result) {
        if (error) {
            return cb (error);
        }
        var premName = result.name
